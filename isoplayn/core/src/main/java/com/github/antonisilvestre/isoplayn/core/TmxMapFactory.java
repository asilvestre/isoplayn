...
import com.github.antonisilvestre.JavaXmlToJson.JavaXmlToJson.*;

/**
 * 
 */
public class TmxMapFactory {

	public static void createTmxMapFromFile(String filepath, final Callback<TmxMap> callback) throws ParseTmxException {
	...
	private static TmxMap parseTmxMap(XmlTag tag) throws ParseTmxException {
		...
		HashMap<String, String> attrs = tag.attributes;
	...
	private static TmxElement parseTmxTileLayer(XmlTag tag) throws ParseTmxException {
		...
		if (attrs.containsKey("opacity")) {
			...
			try {
				...
				if (!validOpacity) {
					throw new ParseTmxException(String.format("Tile layer opacity has to be between 0 and 1: %d",
							opacity));
				}
	...
	private static TmxElement parseTmxObjectGroup(XmlTag tag) throws ParseTmxException {
		...
		if (attrs.containsKey("opacity")) {
			...
			try {
				...
				if (!validOpacity) {
					throw new ParseTmxException(String.format("Tile obj group opacity has to be between 0 and 1: %d",
							opacity));
				}
